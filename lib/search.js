...
//
// Tokenized fuzzy match over title, url, tags, aliases, notes, keyword, folder.
// Abbreviation match: "ep" matches "Event Portal" by initials.
// Operators: tag:x site:x in:folder unused:90d added:7d is:untagged is:stale is:unused
//            is:broken is:unverified is:unchecked (from the dead link check)
...
function parseQuery(q) {
  const parsed = { terms: [], phrase: '', tags: [], sites: [], folders: [], unusedDays: null, addedDays: null, flags: [] };
  const raw = String(q || '').trim();
  const words = [];
  for (const part of raw.split(/\s+/).filter(Boolean)) {
    const m = /^(tag|site|in|folder|col|collection|unused|added|is):(.*)$/i.exec(part);
    ...
    else if (op === 'in' || op === 'folder' || op === 'col' || op === 'collection') parsed.folders.push(val);
...
function passesFilters(parsed, entry, opts, now) {
  ...
  for (const f of parsed.folders) if (!String(link.folder || '').toLowerCase().includes(f)) return false;
  ...
  for (const f of parsed.flags) {
    ...
    if (f === 'broken' && !isBroken(link)) return false;
    if (f === 'unverified' && !isUnverified(link)) return false;
    if (f === 'unchecked' && link.check && link.check.at) return false;
  }
  if (opts.tag && !tags.includes(String(opts.tag).toLowerCase())) return false;
  if (opts.folder && !isWithin(link.folder, opts.folder)) return false;
  if (opts.view === 'unfiled' && link.folder) return false;
  if (opts.view) {
    const v = opts.view;
    ...
    if (v === 'broken' && !isBroken(link)) return false;
    // 'trash' is served from an index built over trashed links only; nothing to filter here.
  }
  return true;
}

function isBroken(link) { return Boolean(link.check && link.check.ok === false); }
function isUnverified(link) { return Boolean(link.check && link.check.at && link.check.ok === null); }
...
// opts: { tag, folder, view, limit, staleDays }. folder includes its subfolders.
function search(index, q, opts = {}) {
  ...
  if (!parsed.terms.length) {
    ...
    else if (view === 'trash') results.sort((a, b) => Date.parse(b.link.deleted || 0) - Date.parse(a.link.deleted || 0));
    else if (view === 'broken') results.sort((a, b) => Date.parse((b.link.check && b.link.check.at) || 0) - Date.parse((a.link.check && a.link.check.at) || 0));
...
module.exports = { tokenize, buildIndex, parseQuery, search, frecency, isStale, snapshotStale, isBroken, isUnverified, resolveGo, expandTemplate };
