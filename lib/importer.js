...
function preview(items, store) {
  ...
  return items.map((it) => {
    ...
    seen.set(key, true);
    const tags = [];
    // Tags the rules add are reported separately so the preview can show where they came from.
    const ruleTags = applyRules(it.url, store.rules);
    for (const t of [...(Array.isArray(it.tags) ? it.tags : []), ...ruleTags]) { const s = slugTag(t); if (s && !tags.includes(s)) tags.push(s); }
